...
def save_filtered_examples(examples):
    ...
    positive_examples.sort(
        key=lambda example: get_positive_class(example["predictions"])["prob"],
        reverse=True,
    )

    with jsonlines.open(
        "/app/lamini-earnings-classify/data/filtered_predictions.jsonl", mode="w"
    ) as writer:
